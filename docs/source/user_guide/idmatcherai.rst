...
- The sessions must have the **same identification image size**. Identification networks cannot process images whose sizes are different from the ones used during training. Check :ref:`Knowledge transfer` to know how to set a fixed identification image size for all your tracking sessions.
