...
class ROI_PopUp(QDialog):
    ...
    def initUI(self):
        ...
        PP_button.clicked.connect(lambda: selected("PP"))
        PE_button.clicked.connect(lambda: selected("PE"))
        NP_button.clicked.connect(lambda: selected("NP"))
        NE_button.clicked.connect(lambda: selected("NE"))
...
class ROI_Widget:
    ...
    def end_ROI_mode(self):
        ...
        return "\n".join(
            self.ROI_list.item(i).text() for i in range(self.ROI_list.count())
        )
